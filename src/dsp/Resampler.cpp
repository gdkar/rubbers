...
D_Speex::D_Speex(Resampler::Quality quality, int channels, int maxBufferSize,
    ...
    m_debugLevel(debugLevel)
{
    int q = (quality == Resampler::Best ? 10 : quality == Resampler::Fastest ? 0 : 4);
    if (m_debugLevel > 0) {
...
D_Speex::setRatio(float ratio){
    ...
    int err = speex_resampler_set_rate_frac (m_resampler, denom, num, 48000, 48000);
    //!!! check err
...
//    }
    ...
    if (m_channels > 1) {v_deinterleave(out, m_iout, m_channels, outcount);}
    return outcount;
}
...
Resampler::Resampler(Resampler::Quality quality, int channels,
    ...
    case 2:
#ifdef USE_SPEEX
        d.reset(new Resamplers::D_Speex(quality, channels, maxBufferSize, debugLevel));
#else
        std::cerr << "Resampler::Resampler(" << quality << ", " << channels
                  << ", " << maxBufferSize << "): No implementation available!"
                  << std::endl;
