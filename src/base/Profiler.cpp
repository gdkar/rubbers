...
{
    auto pmi = m_profiles.find(id);
    if (pmi != m_profiles.end()) {
        ++pmi->second.first;
        pmi->second.second += ms;
    } else { m_profiles[id] = TimePair(1, ms); }
    auto wci = m_worstCalls.find(id);
    if (wci != m_worstCalls.end()) { if (ms > wci->second) wci->second = ms;}
    else { m_worstCalls[id] = ms; }
}
void
Profiler::dump()
{
...
Profiler::getReport()
{
    ...
    for (auto i = m_worstCalls.begin(); i != m_worstCalls.end(); ++i) {
        worstmap.insert(TimeRMap::value_type(i->second, i->first));
    }
    snprintf(buffer, buflen, "\nBy total:\n");
    report += buffer;
    for (auto i = totmap.end(); i != totmap.begin(); ) {
        --i;
    ...
    snprintf(buffer, buflen, "\nBy average:\n");
    report += buffer;
    for (auto i = avgmap.end(); i != avgmap.begin(); ) {
        --i;
        snprintf(buffer, buflen, "%-40s  %f ms\n", i->second, i->first);
        report += buffer;
    }
    snprintf(buffer, buflen, "\nBy worst case:\n");
    report += buffer;
    for (auto i = worstmap.end(); i != worstmap.begin(); ) {
        --i;
        snprintf(buffer, buflen, "%-40s  %f ms\n", i->second, i->first);
        report += buffer;
    }
    snprintf(buffer, buflen, "\nBy number of calls:\n");
    report += buffer;
    for (auto i = ncallmap.end(); i != ncallmap.begin(); ) {
        --i;
    ...
    for (auto i = m_profiles.begin(); i != m_profiles.end(); ++i) { profileNames.insert(i->first); }
    for (auto i = profileNames.begin(); i != profileNames.end(); ++i) {
        auto j = m_profiles.find(*i);
        if (j == m_profiles.end()) continue;
        const TimePair &pp(j->second);
        snprintf(buffer, buflen, "%s(%d):\n", *i, pp.first);
        report += buffer;
        snprintf(buffer, buflen, "\tReal: \t%f ms      \t[%f ms total]\n", (pp.second / pp.first), (pp.second));
        report += buffer;
        auto k = m_worstCalls.find(*i);
...
Profiler::end()
{
    ...
    auto duration = static_cast<std::chrono::duration<double,std::chono::nanoseconds::period> > ( m_end - m_start );
    add(m_c, duration.count());
    m_ended = true;
}
#else /* NO_TIMING */
